...
enum LibraryObject(title: String):
  ...
  case Movie(title: String, data: Vector[Byte]) extends LibraryObject(title)
...
enum IndexedObject(title: String, index: Int):
  ...
  case Movie(title: String, data: Vector[Byte], index: Int = 42) extends IndexedObject(title, index)

case class IndexedChapter(title: String, number: Int, lines: List[String], index: Int = 9000)